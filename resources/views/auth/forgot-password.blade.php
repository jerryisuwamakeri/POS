...
<head>
    ...
    <title>{{ config('app.name', 'Cutietyha POS') }} - Forgot Password</title>
...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-old-lace via-brand-off-yellow to-brand-albescent-white py-8 sm:py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="max-w-md w-full space-y-6 sm:space-y-8 relative z-10">
        ...
        <div class="text-center">
            ...
            <p class="text-brand-akaroa font-medium text-sm sm:text-base">No worries, we'll send you reset instructions</p>
        </div>
        
        <!-- Status Messages -->
        @if (session('status'))
            <div class="bg-gradient-to-r from-brand-teak/20 to-brand-indian-khaki/20 border-2 border-brand-teak/30 rounded-xl p-4 text-brand-husk font-medium text-sm sm:text-base">
                {{ session('status') }}
            </div>
        @endif

        <!-- Forgot Password Form -->
        <form class="mt-6 sm:mt-8 space-y-5 sm:space-y-6 bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-brand-albescent-white shadow-2xl" method="POST" action="{{ route('password.email') }}">
            @csrf
